...
The secure and effective utilization of MetaCentrum resources is possible only if some operational rules are fulfilled. Study the security rules for work in MetaCentrum and the network carefully.
...
5. Usage of non-secured protocols, telnet, FTP, etc., at other machines is not recommended as well. If this authentication method is used, one has to take into account the lower security level of the specific account (and disclosure of the password in open form transferred by the network). It is forbidden to use such an account to log in to MetaCentrum through Kerberos or SSH protocol due to the danger of security breach using a weakly secured account followed by usage of fake SSH or fake Kerberos with a trojan horse and, consequently, the danger of breach of MetaCentrum accounts.
