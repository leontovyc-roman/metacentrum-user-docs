...
title: Kerberos
---
[Kerberos](https://en.wikipedia.org/wiki/Kerberos_(protocol)) is a single sign-on system. Installing Kerberos on your PC means you don't have to fill in your password with every login attempt. You login only once, the Kerberos server issues a ticket which is stored in your computer. During the validity of the ticket, you can log in to every frontend, compute node or storage without entering a password. 

## Linux

### Install Kerberos

On Ubuntu or another Debian-like system, you can install Kerberos using the apt command.
...
You can also make the change of SSH client configuration in a file `/etc/ssh/ssh_config`, it affects the settings of all users on your system.
...
# You will be asked to fill in the password
kinit user123@META 
...
It means that you have a valid ticket for 24 hours to META (Metacentrum).
...
<Callout  type="info" title="Tip" icon="💡">
    If you cannot login try running the command `ntpdate tik.cesnet.cz` to synchronize the clock.
</Callout>

## Windows

<Callout  type="info" title="Tip" icon="💡">
    If you have Windows subsystem for Linux (WSL) installed, you can follow the instructions for Linux.
</Callout>
...
You have to enable hidden items to see the ProgramData folder needed in the next step. 

![pic](/img/meta/access/win-kerb-1.png)
...
![pic](/img/meta/access/win-kerb-2.png)

**Get a new ticket**

![pic](/img/meta/access/win-kerb-3.png)

**Fill in MetaCentrum username and password**

![pic](/img/meta/access/win-kerb-4.png)

and click OK

You should see a valid Kerberos ticket now and if you did everything right, you should not need to type in your password during the login phase (e.g. when using PuTTY):

![pic](/img/meta/access/win-kerb-5.png)
